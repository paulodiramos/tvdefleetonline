...
if __name__ == "__main__":
    exit(main())
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    ...
                    self.log_result("Test-1-Excel-Import", True, 
                                  f"✅ Status 200, Sucessos: {sucesso}, Erros: {erros}")
                    return True
                else:
                    self.log_result("Test-1-Excel-Import", False, 
            ...
            self.log_result("Test-1-Excel-Import", False, f"❌ Error: {str(e)}")
            return False
    ...
    def test_2_mongodb_data_verification(self, headers):
        ...
        if not hasattr(self, 'excel_import_result'):
            self.log_result("Test-2-MongoDB-Verification", False, "❌ No import result from Test 1")
            return False
        
        sucesso = self.excel_import_result.get("sucesso", 0)
        if sucesso == 0:
            self.log_result("Test-2-MongoDB-Verification", True, 
                          "✅ Skipped - No successful imports to verify (expected if no vehicles with CardCode)")
            return True
        ...
        try:
            ...
            self.log_result("Test-2-MongoDB-Verification", True, 
                          f"✅ {sucesso} registos importados com sucesso - dados devem estar no MongoDB")
            return True
            
        except Exception as e:
            self.log_result("Test-2-MongoDB-Verification", False, f"❌ Error: {str(e)}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for vehicle in vehicles:
                    if vehicle.get("cartao_frota_eletric_id") == test_cardcode:
                        vehicle_found = True
                        print(f"  ✅ Veículo encontrado: {vehicle.get('marca')} {vehicle.get('modelo')} - {vehicle.get('matricula')}")
                        print(f"  - CardCode: {vehicle.get('cartao_frota_eletric_id')}")
                        break
                
                if vehicle_found:
                    self.log_result("Test-3-Vehicle-CardCode", True, 
                                  f"✅ Veículo com CardCode {test_cardcode} encontrado")
                    return True
                else:
                ...
                self.log_result("Test-3-Vehicle-CardCode", False, 
                              f"❌ Cannot get vehicles: {response.status_code}")
                return False
                
        except Exception as e:
            self.log_result("Test-3-Vehicle-CardCode", False, f"❌ Error: {str(e)}")
            return False
    ...
    def create_test_vehicle_with_cardcode(self, headers, cardcode):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("Test-3-Vehicle-CardCode", True, 
                              f"✅ Veículo de teste criado com CardCode {cardcode}")
                return True
            else:
                print(f"  ❌ Falha ao criar veículo: {response.status_code}")
                self.log_result("Test-3-Vehicle-CardCode", False, 
                              f"❌ Veículo com CardCode não encontrado e não foi possível criar")
                return False
                
        except Exception as e:
            self.log_result("Test-3-Vehicle-CardCode", False, f"❌ Error creating vehicle: {str(e)}")
            return False
    ...
    def test_4_error_messages_verification(self, headers):
        ...
        try:
            ...
            # Create Excel with invalid CardCode
            ...
            if response.status_code == 200:
                ...
                if erros > 0 and erros_detalhes:
                    ...
                    if "Veículo não encontrado com CardCode" in error_message and "Cartão Frota Elétrico ID" in error_message:
                        self.log_result("Test-4-Error-Messages", True, 
                                      "✅ Mensagem de erro clara e informativa")
                        return True
                    else:
                        self.log_result("Test-4-Error-Messages", False, 
                                      f"❌ Mensagem de erro não é clara: {error_message}")
                        return False
                else:
                    self.log_result("Test-4-Error-Messages", False, 
                                  "❌ Esperado erro com CardCode inválido, mas não houve erros")
                    return False
            else:
                self.log_result("Test-4-Error-Messages", False, 
            ...
            self.log_result("Test-4-Error-Messages", False, f"❌ Error: {str(e)}")
            return False
