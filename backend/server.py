from fastapi import FastAPI, APIRouter, HTTPException, Depends, UploadFile, File, Form, Body, Response
...
from routes.configuracoes import router as configuracoes_router, router_legacy as configuracoes_legacy_router
...
async def process_uber_csv(file_content: bytes, parceiro_id: str, periodo_inicio: str, periodo_fim: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for row in csv_reader:
            ...
            # Rendimentos - usar coluna 'Pago a si' que é o valor líquido pago ao motorista
            for col_name in ['Pago a si', 'Pago a si:Os seus rendimentos', 'Os seus rendimentos', 'Rendimentos', 'Total pago']:
                if col_name in row and row[col_name]:
                    rendimentos = parse_float(row[col_name])
                    logger.debug(f"Usando coluna '{col_name}' para rendimentos: {rendimentos}")
                    break
...
# NOTE: Gestores endpoints moved to routes/gestores.py

# NOTE: Pagamentos comprovativo endpoint moved to routes/pagamentos.py
...
# Vehicle document uploads moved to routes/vehicles.py
...
# NOTE: Reports endpoints moved to routes/reports.py (ROI, Dashboard, Parceiro reports)

# NOTE: Pagamentos endpoints moved to routes/pagamentos.py
...
# NOTE: Endpoint /parceiros/{parceiro_id}/estatisticas moved to routes/parceiros.py

# Parceiros certidao/meu-plano/crud endpoints moved to routes/parceiros.py
...
# Planos CRUD endpoints moved to routes/planos.py
...
# NOTE: Exemplos de contrato endpoints moved to routes/contratos.py
...
# ==================== SUBSCRIPTION/PLANOS ENDPOINTS (MOVED TO routes/planos.py) ====================
...
async def gerar_relatorio_semanal(
    ...
    def debug_log(msg):
        with open('/tmp/debug_relatorio.log', 'a') as f:
            f.write(f"[{datetime.now().isoformat()}] {msg}\n")
    
    debug_log(f"INICIO: Gerando relatório para motorista {motorista_id}")
    ...
    debug_log(f"Incluir Via Verde: {incluir_via_verde}, Config exists: {config is not None}, motorista_id: {motorista_id}")
...
# NOTE: GET /relatorios/semanal/{relatorio_id} moved to routes/relatorios.py
# NOTE: POST /relatorios/semanal/{relatorio_id}/enviar moved to routes/relatorios.py
...
# NOTE: Endpoints duplicados de relatórios removidos - usar routes/relatorios.py
# Removidos: /relatorios/para-verificar, /relatorios/para-pagar, 
#            /relatorios/semanais-para-pagamento, /relatorios/semanais-todos
#            /relatorios/semanal/{id}/marcar-pago, PUT /relatorios/semanal/{id}
#            /relatorios/semanal/{id}/aprovar, /relatorios/semanal/{id}/rejeitar
...
# NOTE: Endpoints de upload de recibos/comprovativos duplicados - usar routes/relatorios.py
# Removidos: POST /relatorios/semanal/{id}/upload-recibo
#            POST /relatorios/semanal/{id}/aprovar-analise
#            GET /relatorios/recibos/{filename}
#            POST /relatorios/semanal/{id}/upload-comprovativo
#            GET /relatorios/comprovativos/{filename}
...
async def importar_combustivel_excel(
    ...
    try:
        ...
        # Ler linha 4 como cabeçalho (primeiras 3 linhas são ignoradas)
        ...
        for row_num, row_values in enumerate(sheet.iter_rows(min_row=5, values_only=True), start=5):
            try:
                ...
                # CARTÃO do combustível vai para cartao_frota_fossil_id
                if cartao_via_verde:
                    vehicle = await db.vehicles.find_one(
                        {"$or": [
                            {"cartao_frota_id": cartao_via_verde},
                            {"cartao_frota_fossil_id": cartao_via_verde}
                        ]},
                        {"_id": 0}
                    )
                    if vehicle:
                ...
                if not vehicle and desc_cartao:
                    ...
                    # Se não encontrou por matrícula, tentar por cartao_frota_id ou cartao_frota_fossil_id
                    if not vehicle:
                        vehicle = await db.vehicles.find_one(
                            {"$or": [
                                {"cartao_frota_id": desc_cartao},
                                {"cartao_frota_fossil_id": desc_cartao}
                            ]},
                            {"_id": 0}
                        )
                        if vehicle:
                            logger.info(f"✅ Veículo encontrado por DESC. CARTÃO: {desc_cartao}")
    ...
    try:
        ...
        for row_num, row_values in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            try:
                ...
                # Se o motorista tem acumular_viaverde activo, usar o acumulado para pagar esta portagem
                if motorista and liquid_value > 0:
                    config_financeira = motorista.get("config_financeira", {})
                    if config_financeira.get("acumular_viaverde", False):
                        acumulado_actual = config_financeira.get("viaverde_acumulado", 0)
                        
                        if acumulado_actual >= liquid_value:
                            # Tem acumulado suficiente - descontar do acumulado
                            novo_acumulado = acumulado_actual - liquid_value
                            await db.motoristas.update_one(
                                {"id": motorista["id"]},
                                {"$set": {"config_financeira.viaverde_acumulado": round(novo_acumulado, 2)}}
                            )
                            # Marcar portagem como paga pelo acumulado
                            await db.portagens_viaverde.update_one(
                                {"id": documento["id"]},
                                {"$set": {"pago_pelo_acumulado": True, "valor_usado_acumulado": liquid_value}}
                            )
                            logger.info(f"💰 Via Verde pago pelo acumulado: €{liquid_value:.2f} (restante: €{novo_acumulado:.2f}) - {motorista.get('name')}")
                        else:
                            # Acumulado insuficiente - usar o que tem e registar diferença
                            diferenca = liquid_value - acumulado_actual
                            await db.motoristas.update_one(
                                {"id": motorista["id"]},
                                {"$set": {"config_financeira.viaverde_acumulado": 0}}
                            )
                            await db.portagens_viaverde.update_one(
                                {"id": documento["id"]},
                                {"$set": {
                                    "pago_pelo_acumulado": True, 
                                    "valor_usado_acumulado": acumulado_actual,
                                    "valor_a_pagar": diferenca
                                }}
                            )
                            logger.info(f"💰 Via Verde parcialmente pago: €{acumulado_actual:.2f} do acumulado, €{diferenca:.2f} a pagar - {motorista.get('name')}")
    ...
    try:
        ...
        for motorista_info in motoristas_com_dados:
            ...
            relatorio = {
                ...
                "aluguer_veiculo": aluguer_veiculo_valor,  # Valor semanal do veículo
                "tipo_contrato_veiculo": tipo_contrato_veiculo,  # "aluguer" ou "comissao"
    ...
    try:
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                # Para Uber, Bolt resumo semanal e GPS, usar data atual se não houver data específica
                if not data and (plataforma == 'uber' or plataforma == 'gps' or is_bolt_resumo_semanal):
...
# NOTE: Users CRUD endpoints moved to routes/users.py
# Includes: /users/pending, /users/all, /users/{id}/approve, /users/{id}/set-role, 
#           /users/{id}/status, /users/{id}, /users/{id}/reset-password
...
# ==================== PLANOS MOTORISTA/SISTEMA/PARCEIRO ENDPOINTS (MOVED TO routes/planos.py) ====================
...
app.include_router(configuracoes_legacy_router, prefix="/api")
...
app.include_router(storage_config_router, prefix="/api")
app.include_router(modulos_router, prefix="/api")
app.include_router(recibos_router, prefix="/api")
app.include_router(empresas_faturacao_router, prefix="/api")
...
app.include_router(cartoes_frota_router, prefix="/api")
...
app.include_router(whatsapp_web_router, prefix="/api")
...
app.include_router(prepagamento_router, prefix="/api")
...
app.include_router(contabilistas_router, prefix="/api")
app.include_router(migracao_router, prefix="/api/admin")
...
async def get_documentos_pendentes(current_user: Dict = Depends(get_current_user)):
    """Listar todos os utilizadores não aprovados (com ou sem documentos pendentes) (Admin/Parceiro)"""
    user_role = current_user["role"]
...
# NOTA: Endpoints DELETE e PATCH /notificacoes/{notificacao_id} migrados para routes/notificacoes.py
...
# NOTE: POST /configuracoes/categorias-plataformas moved to routes/configuracoes.py
...
# Endpoint para download do backup completo
@app.get("/api/download-backup-completo")
async def download_backup_completo():
    backup_path = ROOT_DIR / "static" / "tvdefleet_backup_completo.archive"
    if backup_path.exists():
        return FileResponse(
            path=str(backup_path),
            filename="tvdefleet_backup_completo.archive",
            media_type="application/octet-stream"
        )
    raise HTTPException(status_code=404, detail="Backup not found")
...
# Endpoint para download do backup final
@app.get("/api/download-backup-final")
async def download_backup_final():
    backup_path = ROOT_DIR / "static" / "tvdefleet_backup_final.archive"
    if backup_path.exists():
        return FileResponse(
            path=str(backup_path),
            filename="tvdefleet_backup_final.archive",
            media_type="application/octet-stream"
        )
    raise HTTPException(status_code=404, detail="Backup not found")
