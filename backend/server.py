...
# ==================== EXTRAS DO MOTORISTA (Dívidas, Caução, Danos) ====================

@api_router.get("/extras-motorista")
...
@api_router.post("/extras-motorista")
...
@api_router.put("/extras-motorista/{extra_id}")
...
@api_router.delete("/extras-motorista/{extra_id}")
...
@api_router.post("/vehicles/{vehicle_id}/registar-venda")
async def registar_venda_veiculo(
    vehicle_id: str,
    venda: dict = Body(...),
    current_user: dict = Depends(get_current_user)
):
    """Regista a venda de um veículo"""
    try:
        if current_user['role'] not in ['admin', 'gestao', 'parceiro']:
            raise HTTPException(status_code=403, detail="Acesso negado")
        
        # Verificar se veículo existe
        veiculo = await db.vehicles.find_one({'id': vehicle_id})
        if not veiculo:
            raise HTTPException(status_code=404, detail="Veículo não encontrado")
        
        # Verificar permissão
        if current_user['role'] == 'parceiro' and veiculo.get('parceiro_id') != current_user['id']:
            raise HTTPException(status_code=403, detail="Sem permissão para este veículo")
        ...
        await db.vehicles.update_one(
            ...
            {'$set': {
                'vendido': True,
                'data_venda': venda.get('data_venda', now.strftime('%Y-%m-%d')),
                'valor_venda': float(venda['valor_venda']),
                'comprador_nome': venda.get('comprador_nome'),
                'comprador_contacto': venda.get('comprador_contacto'),
                'status': 'vendido',
                'updated_at': now
            }}
        )
        
        # Registar como receita do parceiro
        receita_id = str(uuid.uuid4())
        await db.partner_revenues.insert_one({
            'id': receita_id,
            'parceiro_id': veiculo.get('parceiro_id'),
            'tipo': 'venda_veiculo',
            'descricao': f"Venda do veículo {veiculo.get('matricula')}",
            'valor': float(venda['valor_venda']),
            'data': venda.get('data_venda', now.strftime('%Y-%m-%d')),
            'vehicle_id': vehicle_id,
        ...
        logger.error(f"Erro ao registar venda: {e}")
        raise HTTPException(status_code=500, detail=str(e))
