...
5. **URIs de redireccionamento autorizados:**
   ```
   https://fleet-reports-dash.preview.emergentagent.com/api/storage-config/oauth/google_drive/callback
   ```
...
1. Na tab **Settings**, em **OAuth 2**, adicione o Redirect URI:
   ```
   https://fleet-reports-dash.preview.emergentagent.com/api/storage-config/oauth/dropbox/callback
   ```
   (Substitua pelo URL real da sua aplicação)
...
3. **URI de Redireccionamento:**
   ...
   - URL: 
   ```
   https://fleet-reports-dash.preview.emergentagent.com/api/storage-config/oauth/onedrive/callback
   ```
4. Clique **Registar**
